  ...
  const handleSendChat = () => {
    ...
    setTimeout(() => {
      ...
      if (msg.includes('salom') || msg.includes('hello') || msg.includes('привет') || msg.includes('hi')) {
        ...
        reply = lang === 'ru' ? '🔑 Пароль: мин 8 символов + большая + маленькая + цифра + спецсимвол (@!#)\nЗабыли? → "Забыл пароль" → email → код (email orqali kod keladi)' : lang === 'en' ? '🔑 Password: min 8 + uppercase + lowercase + number + special (@!#)\nForgot? → "Forgot" → email → code (code via email)' : "🔑 Parol: 8+ belgi + katta harf + kichik harf + raqam + @!#\nUnutdingiz? → 'Parolni unutdingizmi?' → email → kod (code via email)";
